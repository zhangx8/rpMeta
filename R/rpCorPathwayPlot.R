#' Create pathway enrichment plots for correlation analysis 
#'
#' `rpCorPathwayPlot()` create a combined RNA/protein/meta pathway enrichment analysis bubble plot for correlaiton analysis.
#'
#' @param df pathway enrichment result data frame
...
#' @return None
#' @export
rpCorPathwayPlot <- function(df, result_dir, continuous_var, pathways = "Hallmark", FDR_cutoff = 0.05, max_pathways = 30, X11 = "No") {
    library(data.table)
    library(ggplot2)
    library(dplyr)
    library(ggpubr)
    ...
    # limit & reorder by Meta padj to top n, then order by NES (ascending order, negative to positive)
    meta_df <- combined_df %>%
    ...
    # filter by pathway_levels, NES in descending order (positive to negative) 
    combined_df <- combined_df %>%
      filter(pathway %in% pathway_levels) %>%
      mutate(pathway = factor(pathway, levels = pathway_levels))

    # Generate Bubble Plot
    pdf_file <- file.path(result_dir, paste0("rpMeta.rpCorPathway.", tolower(pathways), ".pdf"))
    if (X11=="No") pdf(pdf_file, width = 10, height = 12)
    title <- paste0('rpCorPathway plot (', 'continuous variable: ', continuous_var, ')')
    ...
    message("rpCorPathway plot saved to: ", pdf_file)
}
